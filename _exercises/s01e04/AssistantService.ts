...
export class AssistantService {
    ...
    async extractQueries(messages: ChatCompletionMessageParam[], trace: LangfuseTraceClient, user: string): Promise<string[]> {
        ...
        try {
            const thread: ChatCompletionMessageParam[] = [
                { role: "system", content: extractSearchQueriesPrompt({memoryStructure, knowledge: defaultKnowledge, user}) },
                ...messages
            ];
    ...
    async shouldLearn(messages: ChatCompletionMessageParam[], memories: string, trace: LangfuseTraceClient, user: string): Promise<ShouldLearnResponse | ParsingError> {
        const thread: ChatCompletionMessageParam[] = [
            { role: "system", content: shouldLearnPrompt({memoryStructure, knowledge: defaultKnowledge, memories, user}) },
            ...messages
        ];
    ...
    async answer(config: {
        ...
        const { messages, memories = '', knowledge = defaultKnowledge, learnings = '', user = 'AdamRS>', ...restConfig } = config;
        const systemMessage: ChatCompletionMessageParam = { role: 'system', content: `As Agent X, you're speaking to ${user}. Answer based on the following memories:\n${memories} and general knowledge:\n${knowledge}. Learnings from the conversation:\n${learnings}` };
        const messagesWithSystem = [systemMessage, ...messages.filter(msg => msg.role !== 'system')];

        const generation = this.langfuseService.createGeneration(trace, "ModelAnswer", { messages: messagesWithSystem, ...restConfig });
